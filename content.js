// ============================================
  ...
  parent.hasAttribute('tabindex')) {
      ...
      if (expandedContent) {
        debugLog(`  Waiting for content to expand...`);
      }
      ...
      if (DEBUG && allLinks.length > 0) {
        ...
        Array.from(allLinks)
          ...
          .forEach((link, i) => {
            ...
            debugLog(`  ${marker} Link ${i}: ${href.substring(href.indexOf('instagram.com/') + 
  14, 80)} - Text: "${link.textContent.substring(0, 20)}"`);
          });
        ...
        keywords.forEach((keyword) => {
          ...
          if (partialHashtagMatch) {
            if (tagText.includes(keywordLower)) {
              ...
              debugLog(
                `  ✓ SPOILER DETECTED! Partial hashtag link match: "${keyword}" in 
  "#${tagText}"`
              );
            } else {
          ...
          keywords.forEach((keyword) => {
            ...
            if (partialHashtagMatch) {
              ...
              if (tagText.includes(keywordLower)) {
                ...
                debugLog(
                  `  ✓✓ SPOILER DETECTED! Partial text hashtag match: "${keyword}" found in 
  "#${tagText}"`
                );
              } else {
                debugLog(
              ...
              if (tagText === keywordLower) {
                ...
                debugLog(
                  `  ✓✓ SPOILER DETECTED! Exact text hashtag match: "${keyword}" = 
  "#${tagText}"`
                );
              } else {
                debugLog(`  ✗ No exact match: "${keyword}" != "#${tagText}"`);
        ...
        debugLog(
          `  Note: This is normal if hashtags are rendered as links, or if there are no 
  hashtags`
        );
      }
